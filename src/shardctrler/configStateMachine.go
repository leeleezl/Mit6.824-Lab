...
func (cf *MemoryConfigStateMachine) Join(groups map[int][]string) Err {
	...
	//加入 对于每个 gid -> servers 列表，如果配置中不存在那么就加入到配置中
	...
	//load balance 每次选择一个拥有 shard 数最多的 raft 组和一个拥有 shard 数最少的 raft，将前者管理的一个 shard 分给后者，周而复始，直到它们之前的差值小于等于 1 且 0 raft 组无 shard 为止
	for {
